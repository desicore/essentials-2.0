...
1. **Plan (Astra)** — turn the header above into `courses/08-recordings-course-reports/plan.md`: the six questions as `Q1`..`Q6` (verbatim from the header), 8–12 concrete search queries (screens + flows, per source) mapped to questions, the product shortlist, and the agent split below. Create `courses/08-recordings-course-reports/references.stub.json` with the module header + questions in the `schema.md` format (see Pipeline).
2. **Mobbin fetch agents (Luna, 3 total, one per 2 questions; parallel as slots allow)** — each runs `search_screens` (`mode: "deep"`, `platform: "web"` first, then `"ios"` for the best one or two queries) and `search_flows` for its queries, with the shared `task_intent`. For every kept result it appends a reference object (schema below, `question` set, `take` = one evidence-based sentence, `selected: false`) to `courses/08-recordings-course-reports/mobbin.Qa-Qb.json`. After **each** search call it runs `fetch-assets.mjs` on that file so the per-call Mobbin image links are downloaded before they expire. Returns ≤200 words: products, counts per question, anything surprising. No analysis.
3. **Refero fetch agent (Luna, 1)** — same job via the HTTP helper (`refero-q.mjs`, see Pipeline), writes `courses/08-recordings-course-reports/refero.json`. Refero preview URLs are stable but some return 403; skip those silently.
...
5. **Pattern agents (Sol, 1 per question, 6 total; parallel as slots allow)** — each reads the references and local assets for one question and writes `courses/08-recordings-course-reports/notes/Qn.md` (≤350 words): how each product answers the question, the flow step by step where a flow exists, what is distinctive, one candidate counter-example. Evidence only, cite reference ids.
...
### OpenAI model routing and token budget

- **Astra (`gpt-6-astra`)** — orchestration, planning, merge scripts, synthesis and final QA.
- **Luna (`gpt-5.6-luna`, `reasoning_effort: "low"`)** — bounded Mobbin and Refero fetching tasks.
- **Sol (`gpt-5.6-sol`, `reasoning_effort: "medium"`)** — evidence comparison and per-question pattern notes.
- Set `model` explicitly on every `spawn_agent` call and use `fork_turns: "none"` so workers do not inherit the orchestrator's full conversation or default to Astra. Give each worker a self-contained brief with its assigned questions, exact file paths, relevant context and rules, schema, script commands, and output limits. Include any applicable context-mode routing instructions explicitly.
- Respect the runtime's concurrency limit, including the orchestrator. With four total slots, run at most three workers at once: schedule the fourth fetch task and the six pattern tasks in batches, reusing suitable workers where possible.
- Keep raw search results and screenshots in worker contexts and local files. Return only the required compact summaries and file paths; Astra reads the six notes for synthesis. Retain the existing 200-word fetch summaries and 350-word pattern-note limits.
- Use Luna for fetching and Sol for analysis by default. Escalate only a specific failed or ambiguous task to the next tier, passing its saved files and a short explanation instead of repeating completed searches.
