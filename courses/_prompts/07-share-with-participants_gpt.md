...
OpenAI model assignments: **Astra (`gpt-6-astra`)** for planning and synthesis; **Luna (`gpt-5.6-luna`, low reasoning)** for fetching and reference extraction; **Sol (`gpt-5.6-sol`, medium reasoning)** for pattern analysis. Set the model explicitly when spawning each sub-agent so fetch and pattern tasks do not inherit Astra. With `collaboration.spawn_agent`, use `fork_turns: "none"` and provide a self-contained task with only the relevant questions, paths, schema, and commands. Keep raw results in files and return only the bounded summaries below to minimise context and token usage. Run independent tasks in parallel up to the available agent limit, batching the remaining tasks; reuse completed agents of the same model for subsequent tasks when possible.

1. **Plan (Astra)** — turn the header above into `courses/07-share-with-participants/plan.md`: the six questions as `Q1`..`Q6` (verbatim from the header), 8–12 concrete search queries (screens + flows, per source) mapped to questions, the product shortlist, and the agent split below. Create `courses/07-share-with-participants/references.stub.json` with the module header + questions in the `schema.md` format (see Pipeline).
...
3. **Refero fetch agent (Luna, 1)** — same job via the HTTP helper (`refero-q.mjs`, see Pipeline), writes `courses/07-share-with-participants/refero.json`. Refero preview URLs are stable but some return 403; skip those silently.
...
5. **Pattern agents (Sol, 1 per question, 6 total, in parallel batches as capacity allows)** — each reads the references and local assets for one question and writes `courses/07-share-with-participants/notes/Qn.md` (≤350 words): how each product answers the question, the flow step by step where a flow exists, what is distinctive, one candidate counter-example. Evidence only, cite reference ids.
