...
Cross-tenant synchronization is currently not supported in [external tenants](https://learn.microsoft.com/en-us/entra/external-id/customers/overview-customers-ciam).
...
7. Select **Provision** at the bottom of the page.
	![Screenshot that shows the Microsoft Entra admin center UI for provisioning a user on demand.](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/media/provision-on-demand/on-demand-provision-user.png)
...
- Ensure that you've provided valid credentials, such as the secret token and tenant URL, to the target system. The required credentials vary by application. For detailed configuration tutorials, see the [tutorial list](https://learn.microsoft.com/en-us/entra/identity/saas-apps/tutorial-list).
...
Next, the provisioning service determines whether the user is in [scope](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/how-provisioning-works#scoping) for provisioning. The service considers aspects such as:
...
- Make sure that you've defined a valid scoping role. For example, avoid using the [Greater\_Than operator](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/define-conditional-rules-for-provisioning-user-accounts#create-a-scoping-filter) with a noninteger value.
- If the user doesn't have the necessary role, review the [tips for provisioning users assigned to the default access role](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/application-provisioning-config-problem-no-users-provisioned#provisioning-users-assigned-to-the-default-access-role).
...
![Screenshot that shows the successful on-demand provisioning of a user.](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/media/provision-on-demand/success-on-demand-provision.png)
...
- Failures for exporting changes can vary greatly. Check the [documentation for provisioning logs](https://learn.microsoft.com/en-us/entra/identity/monitoring-health/howto-analyze-provisioning-logs#error-codes) for common failures.
...
- [Troubleshooting provisioning](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/troubleshoot)