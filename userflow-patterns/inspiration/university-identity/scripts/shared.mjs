...
export async function readReferences(input) {
  ...
  const questions = (data.questions ?? []).map((q, i) => {
    ...
    ids.add(id);
    return { ...q, id, title: string(q?.title) || id, answer: string(q?.answer) };
  ...
  const references = (data.references ?? []).map((ref, i) => {
    ...
    refIds.add(id);
    return {
      ...ref, id, source: ['mobbin', 'refero', 'documentation'].includes(ref?.source) ? ref.source : 'unknown',
  ...
  return { ...data, module: string(data.module), title: string(data.title) || 'Design references', intro: string(data.intro), questions, references };
}
